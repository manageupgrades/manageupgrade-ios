...
public class SwiftManageUpgradesPlugin: NSObject, FlutterPlugin {
    public static func register(with registrar: FlutterPluginRegistrar) {
        let channel = FlutterMethodChannel(name: "manageupgrades", binaryMessenger: registrar.messenger())
        let instance = SwiftManageUpgradesPlugin()
    ...
    public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        case "checkAppStatus":
            if let args = call.arguments as? [String: Any],
               ...
               let googleId = args["googleid"] as? String,
               let appleId = args["appleid"] as? String {
                
                checkAppStatus(projectId: projectId, 
                             version: version, 
                             platform: platform, 
                             apiKey: apiKey,
                             googleId: googleId,
                             appleId: appleId) { response in
                    result(response)
                }
            } else {
    ...
    private func checkAppStatus(projectId: String, 
                              version: String, 
                              platform: String, 
                              apiKey: String,
                              googleId: String,
